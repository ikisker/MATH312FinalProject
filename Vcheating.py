...
def generate_noise():
    global u
    global noise1_wav_obj, noise1_wavio_obj,noise1_data
    ...
    #create a wavio object for the noise
    noise1_wavio_obj = wavio.read("noise.wav")
    noise1_data = sample_wavio_obj.data

    # Plot the noise waveform
    plot_waveform(noise_canvas, noise1_data, "Noise Signal")
...
def combine_data():
    global audio_data, noise1_data, combined_data, combined_wav_obj, sample_wav_obj

    
    # Check if audio data and noise data are available
    if audio_data is None or noise1_data is None:
        print("Audio data or noise data is missing.")
        return
    
    # Check if the lengths of audio data and noise data are the same
    if len(audio_data) != len(noise1_data):
    ...
    combined_data = audio_data + noise1_data
...
def decode_audio():
    ...
    decoded_data = combined_data - noise1_wav_obj
